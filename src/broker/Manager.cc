...
template <class T> decltype(auto) move_if_broker_data(T&& arg)
	{
...
std::string& c_str_safe(std::string& arg)
	{
	return arg;
	}

std::string c_str_safe(std::string_view& arg)
	{
...
std::string RenderMessage(const broker::variant_list& xs)
	{
...
template <class DataOrVariant> std::string RenderMessage(std::string topic, const DataOrVariant& x)
	{
...
void Manager::ProcessMessage(std::string_view topic, broker::zeek::Event& ev)
	...
	if ( ! ev.valid() )
		...
		reporter->Warning("received invalid broker Event: %s",
		                  broker::to_string(ev.as_data()).c_str());
		return;
		}
	...
	DBG_LOG(DBG_BROKER, "Process event: %s (%.6f) %s", c_str_safe(name).c_str(), ts,
  ...
  for ( const auto& arg : args )
	  ...
	  if ( val )
		  ...
		  reporter->Warning("failed to convert remote event '%s' arg #%zu, %s",
			                c_str_safe(name).c_str(), index, msg_addl.c_str());
...
bool Manager::ProcessMessage(std::string_view, broker::zeek::LogCreate& lc)
	...
	if ( ! lc.valid() )
		...
		reporter->Warning("received invalid broker LogCreate: %s",
		                  broker::to_string(lc.as_data()).c_str());
		return false;
		}
...
bool Manager::ProcessMessage(std::string_view, broker::zeek::LogWrite& lw)
	...
	if ( ! lw.valid() )
		...
		reporter->Warning("received invalid broker LogWrite: %s",
		                  broker::to_string(lw.as_data()).c_str());
		return false;
		}
...
bool Manager::ProcessMessage(std::string_view, broker::zeek::IdentifierUpdate& iu)
	...
	if ( ! iu.valid() )
		...
		reporter->Warning("received invalid broker IdentifierUpdate: %s",
		                  broker::to_string(iu.as_data()).c_str());
		return false;
		}
	...
	if ( ! id )
		{
		reporter->Warning("Received id-update request for unknown id: %s",
		                  c_str_safe(id_name).c_str());
		return false;
		}
	...
	if ( ! val )
		...
		reporter->Error("Failed to receive ID with unsupported type: %s (%s)",
		                c_str_safe(id_name).c_str(), type_name(id->GetType()->Tag()));
		return false;
		}
...
void Manager::ProcessStatus(const broker::status& stat)
	...
	switch ( stat.code() )
		...
		default:
			reporter->Warning("Unhandled Broker status: %s", to_string(stat).c_str());
			break;
		}
