...
public:
     ...
     */
    bool IsSameAs(const BaseGauge<BaseType>& other) const noexcept { return *handle == *other.handle; }
...
public:
    ...
    explicit IntGauge(FamilyType& handle, const prometheus::Labels& labels) noexcept : BaseGauge(handle, labels) {}
...
public:
    ...
    explicit DblGauge(FamilyType& handle, const prometheus::Labels& labels) noexcept : BaseGauge(handle, labels) {}
...
public:
    BaseGaugeFamily(std::string_view prefix, std::string_view name, Span<const std::string_view> labels,
                    std::string_view helptext, prometheus::Registry& registry, std::string_view unit = "1",
                    bool is_sum = false)
