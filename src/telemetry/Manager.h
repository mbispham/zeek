...
public:
    ...
    auto CounterFamily(std::string_view prefix, std::string_view name, Span<const std::string_view> labels,
        ...
        if constexpr ( std::is_same<ValueType, int64_t>::value ) {
            ...
            auto int_fam = std::make_shared<IntCounterFamily>(prefix, name, labels, helptext, *registry, unit, is_sum);
            ...
            auto dbl_fam = std::make_shared<DblCounterFamily>(prefix, name, labels, helptext, *registry, unit, is_sum);
    ...
    Counter<ValueType> CounterInstance(std::string_view prefix, std::string_view name, Span<const LabelView> labels,
        ...
        return WithLabelNames(labels, [&, this](auto labelNames) {
            auto family = CounterFamily<ValueType>(prefix, name, convert_labels(labelNames), helptext, unit, is_sum);
    ...
    auto GaugeFamily(std::string_view prefix, std::string_view name, Span<const std::string_view> labels,
        ...
        if constexpr ( std::is_same<ValueType, int64_t>::value ) {
            ...
            auto int_fam = std::make_shared<IntGaugeFamily>(prefix, name, labels, helptext, *registry, unit, is_sum);
            families.push_back(int_fam);
            return int_fam;
        }
        else {
            ...
            auto dbl_fam = std::make_shared<DblGaugeFamily>(prefix, name, labels, helptext, *registry, unit, is_sum);
            families.push_back(dbl_fam);
            return dbl_fam;
        }
    }
    ...
    Gauge<ValueType> GaugeInstance(std::string_view prefix, std::string_view name, Span<const LabelView> labels,
                                   std::string_view helptext, std::string_view unit = "1", bool is_sum = false) {
        return WithLabelNames(labels, [&, this](auto labelNames) {
            auto family = GaugeFamily<ValueType>(prefix, name, convert_labels(labelNames), helptext, unit, is_sum);
            return family.getOrAdd(labels);
        });
    }
    ...
    auto HistogramFamily(std::string_view prefix, std::string_view name, Span<const std::string_view> labels,
        ...
        if constexpr ( std::is_same<ValueType, int64_t>::value ) {
            ...
            auto int_fam =
                std::make_shared<IntHistogramFamily>(prefix, name, labels, helptext, *registry, unit, is_sum);
            families.push_back(int_fam);
            return int_fam;
        }
        else {
            ...
            auto dbl_fam =
                std::make_shared<DblHistogramFamily>(prefix, name, labels, helptext, *registry, unit, is_sum);
            families.push_back(dbl_fam);
            return dbl_fam;
        }
    }
    ...
    Histogram<ValueType> HistogramInstance(std::string_view prefix, std::string_view name, Span<const LabelView> labels,
                                           ...
                                           std::string_view unit = "1", bool is_sum = false) {
        return WithLabelNames(labels, [&, this](auto labelNames) {
            auto family = HistogramFamily<ValueType>(prefix, name, convert_labels(labelNames), default_upper_bounds,
